...
class dynamics_driver():
    ...
    def one_rk_step_mf( self, nproc ):
        ...
        change_mf1RDM = self.delt * mf1rdm_timedep_mod.get_ddt_mf1rdm_serial( self.tot_system, round(self.tot_system.Nele/2) )
            ...
            for b in frag.bathrange:
                for a in frag.bathrange:
                    if( b != a ):
                        ...
                        X_small[ib,ia] = 1.0/( frag.env1RDM_evals[a-frag.Nimp] - frag.env1RDM_evals[b-frag.Nimp] ) * utils.matprod( utils.adjoint(frag.rotmat)[b,:], 1j*change_mf1RDM/self.delt, frag.rotmat[:,a] )
        ...
        if( nproc == 1 ):
            ...
            for ifrag, frag in enumerate(self.tot_system.frag_list):
                ...
                change_CIcoeffs_list.append( -1j * self.delt * applyham_pyscf.apply_ham_pyscf_fully_complex( frag.CIcoeffs, frag.h_emb-Xmat_list[ifrag], frag.V_emb, frag.Nimp, frag.Nimp, 2*frag.Nimp, frag.Ecore ) ) #grr

...
#THE NOTATION FOR THE 1-RDM IS AS FOLLOWS: gamma_ij = < a_j^dag a_i >, WHICH IS THE OPPOSITE OF PYSCF
...
class tdfci():
    ...
    def __init__(self, Nsites, Nelec, h_site, V_site, CIcoeffs, delt, Nstep, Nprint, Ecore=0.0 ):
    ...
    def kernel( self ):
        ...
        #main subroutine for Real-Time propagation of FCI coefficients under a time-independent hamiltonian
        ...
        for step in range( self.Nstep ):
            ...
            if( np.mod( step, self.Nprint ) == 0 ):
                print('Writing data at step ', step, 'and time', current_time, 'for TDFCI calculation')
                self.print_data( current_time )
                sys.stdout.flush()
    
            #Integrate FCI coefficients by a time-step
            self.CIcoeffs = integrators.runge_kutta_pyscf( self.CIcoeffs, self.Nsites, int(self.Nelec/2), int(self.Nelec/2), self.delt, self.h_site, self.V_site, self.Ecore )
    
            #update the current time
            current_time = self.delt * ( step + 1 )
   
        #Print data at final step regardless of Nprint
        print('Writing data at step ', step+1, 'and time', current_time, 'for TDFCI calculation')
        self.print_data( current_time )
        sys.stdout.flush()
    ...
    def print_data( self, current_time ):
        ...
        Etot = fci_mod.get_FCI_E( self.h_site, self.V_site, self.Ecore, self.CIcoeffs, self.Nsites, int(self.Nelec/2), int(self.Nelec/2) )

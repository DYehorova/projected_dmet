...
def calc_omega_mat( system, indxdict, sizeX, Nocc ):
    ...
    chi = np.zeros( [ Nocc, (system.Nsites-Nocc) ] )
    for mu in range(Nocc):
        for nu in range(Nocc,system.Nsites):
            chi[ mu, nu-Nocc ] = 1.0/(system.NOevals[mu]-system.NOevals[nu])
    ...
    omega  = np.einsum( 'ij,pij,qij->pq', chi, unpck_NO_rot[:,:Nocc,Nocc:], big[:,:Nocc,Nocc:] )
    omega += np.einsum( 'ij,pji,qji->pq', chi, unpck_NO_rot[:,Nocc:,:Nocc], big[:,Nocc:,:Nocc] )
